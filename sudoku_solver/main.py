...
class SudokuSolver(nn.Module):
    def __init__(self, embed_dim=32):
        ...
        self.conv1 = nn.Conv2d(in_channels=embed_dim, out_channels=256, kernel_size=3, padding=1)
    ...
    def forward(self, x):
        ...
        logits = self.conv3(x)  # Output shape: (batch_size, 9, 9, 9) in channels-first format: (B, 9, 9, 9) actually is (B, 9, 9, 9) with channels=9.
...
for epoch in range(num_epochs):
    ...
    for input_board, target, mask in dataloader:
        input_board = input_board.to(device)  # Shape: (B, 9, 9)
        target = target.to(device)            # Shape: (B, 9, 9)
        mask = mask.to(device)                # Shape: (B, 9, 9), bool

        optimizer.zero_grad()
        output = model(input_board)  # Output shape: (B, 9, 9, 9) with channels-first: (B, 9, 9, 9)
        # CrossEntropyLoss expects input of shape (B, C, H, W) and target of shape (B, H, W).
        loss_all = criterion(output, target)  # loss_all shape: (B, 9, 9)
        # Compute the mean loss only over the masked cells.
        if mask.sum() > 0:
